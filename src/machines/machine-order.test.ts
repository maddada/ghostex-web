...
import {
  applyRemoteMachineOrder,
  orderMachineConnectionStates,
} from "./machine-order";
...
describe("web remote machine ordering", () => {
  test("pins local first and applies a complete remote order", () => {
    ...
    expect(applyRemoteMachineOrder([local, alpha, beta], ["beta", "alpha"]))
      .toEqual([local, beta, alpha]);
  });
  ...
  test("projects connection state in catalog order while keeping local first", () => {
    ...
    expect(
      orderMachineConnectionStates(states, [local, beta, alpha])
        .map((state) => state.machine.machineId),
    ).toEqual(["local", "beta", "alpha"]);
  });
});
