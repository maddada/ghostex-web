...
const clientStorage = storageScope(["webSettings"]);
...
export function readWebSettings(): ghostexSettings {
  try {
    const stored = clientStorage.getItem(WEB_SETTINGS_STORAGE_KEY);
...
export function writeWebSettings(settings: ghostexSettings): ghostexSettings {
  ...
  clientStorage.setItem(WEB_SETTINGS_STORAGE_KEY, JSON.stringify(normalized));
