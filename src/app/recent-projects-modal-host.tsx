...
export function RecentProjectsModalHost({ runtime }: { runtime: WebSidebarRuntime }) {
  const [modalState, setModalState] = useState<OpenRecentProjectsModalDetail>();

  useEffect(() => {
    const openModal = (event: WindowEventMap['ghostex-web:openRecentProjectsModal']) => setModalState(event.detail);
