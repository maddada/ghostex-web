...
export function loadWorkspaceLayout(machineId: string, sessions: WorkspaceSession[]): WorkspaceModel {
  const stored = readLayouts().layouts[machineId];
  return isWorkspaceModel(stored) ? reconcileWorkspaceSessions(stored, sessions) : createWorkspaceModel(sessions);
}
